...
│   └── {task}.jsonl
├── plans/                # Plan outputs (generated by plan-worker.mjs at runtime)
│   ├── {task}.md         # AI-generated plan content
│   └── {task}.session    # Session ID for develop step resume
├── worktrees/            # Git worktrees (one per task)
│   └── {task}/
└── lib/                  # Helper library (runtime only)
    ├── ai-helpers.sh     # AI worker functions (plan-aware resume, extract_session_id)
