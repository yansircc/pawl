...
`claude` CLI 的非交互模式是 Worker 的基础：

```bash
# 单次执行
claude -p "implement the login feature" --tools "Bash,Read,Write"

# 管道注入 task 描述
cat task.md | claude -p - --tools "Bash,Read,Write"

# 续接已有会话
claude -p "Fix the test failure" -r $session_id --tools "Bash,Read,Write"

# 结构化输出
claude -p "analyze code" --output-format json

# 指定模型
claude -p "task" --model sonnet --tools "Bash,Read,Write"
```

关键点：
- `-p` 表示非交互 (print) 模式，stdout 直接输出结果
- `-r $session_id` 在同一上下文中继续，保留之前的代码理解
- `--tools` 控制 Worker 可用的工具集
- 退出码反映任务执行结果
...
所有变量在 Worker 进程中均可通过环境变量访问：

| 环境变量 | 模板变量 | 值 | 示例 |
|----------|---------|-----|------|
| `WF_TASK` | `${task}` | 任务名 | `auth-feature` |
| `WF_BRANCH` | `${branch}` | Git 分支 | `wf/auth-feature` |
| `WF_WORKTREE` | `${worktree}` | 工作树路径 | `/repo/.wf/worktrees/auth-feature` |
| `WF_WINDOW` | `${window}` | tmux 窗口名 | `auth-feature` |
| `WF_SESSION` | `${session}` | tmux session 名 | `my-project` |
| `WF_REPO_ROOT` | `${repo_root}` | Git 仓库根目录 | `/home/user/my-project` |
| `WF_STEP` | `${step}` | 当前步骤名 | `develop` |
| `WF_STEP_INDEX` | `${step_index}` | 步骤索引 (0-based) | `1` |
| `WF_BASE_BRANCH` | `${base_branch}` | 基础分支 | `main` |
| `WF_LOG_FILE` | `${log_file}` | JSONL 日志路径 | `/repo/.wf/logs/auth-feature.jsonl` |
| `WF_TASK_FILE` | `${task_file}` | Task 文件路径 | `/repo/.wf/tasks/auth-feature.md` |

Event hook 中还可使用事件特定变量：
